...
void *listener_listen(void *arg) {
  ...
  while (true) {
    ...
    ch = getch(); // Get the pressed key, if many keys are pressed at once, each
                  // call to getch will get the next one in order
    nodelay(stdscr, FALSE); // Disable non-blocking input

    pthread_mutex_lock(
        &listener->mutex); // Lock the listener to search for the key
    for (i = 0; i < listener->size; i++) { // Iterate over the listener keys
      ...
      pthread_mutex_lock(
          &listenerKey->mutex); // Lock the listener key to update its status
      if (!listenerKey->status &&
          listenerKey->key ==
              ch) { // Check if the key is the one being listened to
    ...
    pthread_testcancel(); // Check for cancellation requests, in case of a
                          // listener_destroy call
  }
}

Listener *listener_init() {
  Listener *listener = malloc(
      sizeof(Listener)); // Allocate memory for the listener, and initialize it
  listener->size = 0;
  listener->keys = NULL;
  pthread_mutex_init(&listener->mutex, NULL);
  pthread_cond_init(&listener->cond, NULL);

  pthread_create(&listener->thread, NULL, listener_listen,
                 listener); // Create the listener thread

  pthread_mutex_lock(
      &listener->mutex); // Wait for the listener thread to initialize
  pthread_cond_wait(&listener->cond, &listener->mutex);
  pthread_mutex_unlock(&listener->mutex);
...
void listener_destroy(Listener *listener) {
  pthread_mutex_lock(&listener->mutex); // Lock the listener mutex to free its
                                        // listenerKeys resources

  ListenerKey *listenerKey;
  for (unsigned int i = 0; i < listener->size;
       i++) { // Iterate over the listener keys and free their resources
    listenerKey = listener->keys[i];
    if (listenerKey == NULL)
      continue;
...
ListenerKey *listener_subscribe(Listener *listener, char key) {
  ...
  listener->keys =
      realloc(listener->keys, sizeof(ListenerKey *) * (listener->size + 1));
...
void listener_unsubscribe(Listener *listener, ListenerKey *listenerKey) {
  ...
  listener->keys =
      realloc(listener->keys, sizeof(ListenerKey *) * listener->size);
